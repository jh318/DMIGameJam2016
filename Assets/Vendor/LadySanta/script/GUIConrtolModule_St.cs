...
public class GUIConrtolModule_St : MonoBehaviour {
	...
	private ModuleSetupControl chrModule; // Characte prehab.
	...
	public void LODControlBtn () {
		...
		StartCoroutine (ChangeLOD ("Lady Santa" + surfFix[aIdx], aIdx));
	}
	...
	// Slider for Character Module Control
	public void ModuleSliderValueChanged (int idx) {
		int value = (int)moduleSlider [idx].value;
		chrModule.ModuleSelector (idx, value);
		TextureSliderValueChanged (idx);
		chrModule.UpdateMeshData ();
		chrCtrl.chrAnimator.Rebind();

		string[] surfFix = new string[] {" (High)", " (Low)", " (SD)"};
		string[] labalText = { "Head", "Up", "Down", "Bag" };
		ModelInformationUI.GetComponentInChildren<Text> ().text = "Lady Santa" + surfFix[activeLodIdx] + "\n\n" + chrCtrl.MeshData ();
		moduleSliderLabel[idx].text = labalText[idx] + "  " + value.ToString("D2") + "\n";
	...
	// Slider for Change Texture.
	// テクスチャ変更スライダー。.
	// 텍스쳐 변경 슬라이더.
	public void TextureSliderValueChanged (int idx) {
		int value = (int)textureSlider [idx].value;
		chrModule.ColorSelector (idx, value);

		textureSliderLabel[idx].text = "col " + value.ToString("D2") + "\n";
	}
