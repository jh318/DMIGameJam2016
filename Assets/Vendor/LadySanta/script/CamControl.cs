...
public class CamControl : MonoBehaviour {
	...
	public Transform lookPos; // look point of camera
	...
	public float[] zoom = new float[3]; // Camera's FOV values​​ to zoom.
	public float[] lookPosOffset = new float[3]; // look point offset.
	...
	void Update () {
		...
		float newLookPosY;
		if (lookPosOffsetX) {
			// make offset of look point.
			newLookPosX = Mathf.Lerp (lookPos.localPosition.x, lookPosOffsetX.value, Time.deltaTime * smooth);
			newLookPosY = Mathf.Lerp (lookPos.localPosition.y, lookPosOffset [zoomIdx] + lookPosOffsetY.value, Time.deltaTime * smooth);
		}
		else {
			// make offset of look point.
			newLookPosX = 0;
			newLookPosY = Mathf.Lerp (lookPos.localPosition.y, lookPosOffset [zoomIdx], Time.deltaTime * smooth);
		}
		lookPos.localPosition = new Vector3(newLookPosX, newLookPosY, lookPos.localPosition.z);
